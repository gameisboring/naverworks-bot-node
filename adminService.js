...
const 특정직원출퇴근시간UPDATE = `UPDATE io_status set _in = ?, _out= ?, lastupdate = _out, now = 'OUT', who = 'P' WHERE date_format(lastupdate,'%Y-%m-%d') = date_format(?,'%Y-%m-%d') and name = ?;`

module.exports = class adminService {
  async updateUserIO(name, date, in_time, out_time) {
    const in_datetime = date + ' ' + in_time
    const out_datetime = date + ' ' + out_time

    const result = await pool.query(특정직원출퇴근시간UPDATE, [
      in_datetime,
      out_datetime,
      date,
      name,
